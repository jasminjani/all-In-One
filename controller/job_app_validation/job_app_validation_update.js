            ...
            await con.query(select_preferences, [person_id], async (err, result5) => {
              ...
              await con.query(select_tech_known, [person_id], async (err, result6) => {
                ...
                await con.query(select_lang_known, [person_id], (err, result7) => {
                  ...
                  res.render('./job_app_validation/job_app_validation', { person_id: person_id, data: result, data2: result2, data3: result3, data4: result4, data5: result5, data6: result6, data7: result7 })
                });
              });
            });
          });
    ...
    await con.query(update_basic_detail, [body.first_name, body.last_name, body.designation, body.email, body.phone_no, body.gender, body.relationship_status, body.address_1, body.address_2, body.city, body.state, body.zipcode, body.dob, person_id], async (err, result11) => {
      ...
      for (let i = 0; i < req.body.board_or_university.length; i++) {
        ...
        if ((body.board_or_university[i] == 'select_master')) { }
          ...
          await con.query(update_edu_detail, [education, body.board_or_university[i], body.passing_year[i], body.percentage[i], person_id, education], (err, result12) => {
            if (err) throw err;
      ...
      for (let i = 0; i < req.body.company_name.length; i++) {
        ...
        if (body.company_name[i] == null || body.company_name[i].trim() == '') { }
          ...
          await con.query(update_work_exp, [body.company_name[i], body.past_designation[i], body.start_date[i], body.end_date[i], body.work_id[i]], (err, result13) => {
            if (err) throw err;
      ...
      for (let i = 0; i < req.body.ref_name.length; i++) {
        ...
        if (body.ref_name[i] == null || body.ref_name[i].trim() == '') { }
          ...
          await con.query(update_ref_contact, [body.ref_name[i], body.ref_phone_no[i], body.ref_relation[i], body.reference_id[i]], (err, result14) => {
            if (err) throw err;
          });
        }
      }
      ...
      let update_preferances = `UPDATE preferances SET location_1 = ?, location_2 = ?, location_3 = ?, notice_period_in_month =?, expected_ctc = ?, current_ctc = ?, department = ? WHERE preference_id = ?`;

      await con.query(update_preferances, [body.location_1[0], body.location_1[1], body.location_1[2], body.notice_period_in_month, body.expected_ctc, body.current_ctc, body.department, body.preference_id], (err, result15) => {
        if (err) throw err;
      });

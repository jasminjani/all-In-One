        ...
        await con.query(select_basic_detail, [person_id], async (err, result) => {
            ...
            await con.query(select_edu_detail, [person_id], async (err, result2) => {
                ...
                await con.query(select_work_exp, [person_id], async (err, result3) => {
                    ...
                    await con.query(select_ref_contact, [person_id], (err, result4) => {
                        if (err) throw err;


                        let select_preferences = `SELECT * FROM preferances where person_id = ?`;

                        con.query(select_preferences, [person_id], (err, result5) => {
                            if (err) throw err;


                            let select_tech_known = `SELECT * FROM technology_known where person_id = ?`;

                            con.query(select_tech_known, [person_id], (err, result6) => {
                                if (err) throw err;
                                ...
                                con.query(select_lang_known, [person_id], (err, result7) => {
                                    if (err) throw err;
                                    ...
                                    res.render('./task-9/task-9', { person_id: person_id, data: result, data2: result2, data3: result3, data4: result4, data5: result5, data6: result6, data7: result7 })
                                });
                            });
                        });
                    });
...
task_9.post('/task_9/:id', isAuthorization, (req, res) => {
    ...
    con.connect(async (err) => {
        ...
        await con.query(update_basic_detail, [body.first_name, body.last_name, body.designation, body.email, body.phone_no, body.gender, body.relationship_status, body.address_1, body.address_2, body.city, body.state, body.zipcode, body.dob, person_id], async (err, result11) => {
            ...
            con.query(update_preferances, [body.location_1[0], body.location_1[1], body.location_1[2], body.notice_period_in_month, body.expected_ctc, body.current_ctc, body.department, body.preference_id], (err, result15) => {
                if (err) throw err;
